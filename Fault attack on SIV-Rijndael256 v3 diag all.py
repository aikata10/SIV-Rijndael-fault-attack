...
sbox =  [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, #s-box
...
# Column 4 --------------------------------------------
    ...
    for i in range (len(k12_t)):
        for j in range (256):
            a1=k12_t[i][0]
            a2=k12_t[i][1]
            lhs = inSbox(ot[fault][0][7] ^ j) ^ inSbox(ft[fault][0][7] ^ j)
            rhs = galoisMult(inSbox(ot[fault][1][6] ^ a1) ^ inSbox(ft[fault][1][6] ^ a1),5)^galoisMult(inSbox(ot[fault][2][4] ^ a2) ^ inSbox(ft[fault][2][4] ^ a2),7)#using byte-inter-relation equations
            if lhs==rhs:
                k012.add((j,a1,a2))
    k0123=set()
    for i in k012:
    ...
    for i in k013:
        for j in range(256):
            ...
            rhs = galoisMult(inSbox(ot[fault][2][2] ^ j) ^ inSbox(ft[fault][2][2] ^ j),7)^galoisMult(inSbox(ot[fault][3][1] ^ i[2]) ^ inSbox(ft[fault][3][1] ^ i[2]),4)
            if(lhs==rhs):
    ...
    #print("C01234567 len ----- ",len(C01234567))
    return C01234567
...
def diag3(fault):#Function to return the keyspace if the fault is done in 3rd  Diagonal
    ...
    for i in range (256):
        for j in range (256):
            for k in range (256):
                lhs = inSbox(ot[fault][0][2] ^ i) ^ inSbox(ft[fault][0][2] ^ i)#using byte-inter-relation equations
                rhs = inSbox(ot[fault][1][1] ^ j) ^ inSbox(ft[fault][1][1] ^ j)^inSbox(ot[fault][3][6] ^ k) ^ inSbox(ft[fault][3][6] ^ k)
                if(lhs==rhs):
                    k013.add((i,j,k))
    k0123=set()
    ...
    for i in k013:
        for j in range(256):
            lhs = galoisMult(inSbox(ot[fault][3][6] ^ i[2]) ^ inSbox(ft[fault][3][6] ^ i[2]),3)#using byte-inter-relation equations
            rhs = galoisMult(inSbox(ot[fault][1][1] ^ i[1]) ^ inSbox(ft[fault][1][1] ^ i[1]),2)^galoisMult(inSbox(ot[fault][2][7] ^ j) ^ inSbox(ft[fault][2][7] ^ j),7)
            if(lhs==rhs):
                k0123.add((i[0],i[1],j,i[2]))
    # Merging the keyspaces using  key-schedule equations
    C01234567_t1=set()
    ...
    k0123_t=set()
    for i in k0123:
        lhs = inSbox(ot[fault][0][6] ^ i[0]) ^ inSbox(ft[fault][0][6] ^ i[0])
        rhs = galoisMult(inSbox(ot[fault][2][3] ^ i[2]) ^ inSbox(ft[fault][2][3] ^ i[2]),7)^galoisMult(inSbox(ot[fault][3][2] ^ i[3]) ^ inSbox(ft[fault][3][2] ^ i[3]),4)#using byte-inter-relation equations
        if(lhs==rhs):
            lhs1=galoisMult(inSbox(ot[fault][0][6]^i[0])^inSbox(ft[fault][0][6]^i[0]),5)^galoisMult(inSbox(ot[fault][1][5]^i[1])^inSbox(ft[fault][1][5]^i[1]),7)
...
def diag4(fault):#Function to return the keyspace if the fault is done in 4th  Diagonal
    ...
    for i in range (len(k0123_t)):
            ...
            a1=k0123_t[i][1]
            a2=k0123_t[i][2]
            lhs = inSbox(ot[fault][0][1] ^ a0) ^ inSbox(ft[fault][0][1] ^ a0)#using byte-inter-relation equations
            rhs = galoisMult(inSbox(ot[fault][1][0] ^ a1) ^ inSbox(ft[fault][1][0] ^ a1),5)^galoisMult(inSbox(ot[fault][2][6] ^ a2) ^ inSbox(ft[fault][2][6] ^ a2),7)
            if lhs==rhs:
                t=(k0123_t[i][0],k0123_t[i][1],k0123_t[i][2],k0123_t[i][3])
                k0123.add(t)
    C1 = list(k0123)
...
def diag6(fault):#Function to return the keyspace if the fault is done in 6th  Diagonal
    ...
    for i in range (256):
        for j in k02_t:
                lhs = inSbox(ot[fault][0][3] ^ j[0]) ^ inSbox(ft[fault][0][3] ^ j[0])#using byte-inter-relation equations
                rhs = galoisMult(inSbox(ot[fault][1][2] ^ i) ^ inSbox(ft[fault][1][2] ^ i),5)^galoisMult(inSbox(ot[fault][2][0] ^ j[1]) ^ inSbox(ft[fault][2][0] ^ j[1]),7)
                if(lhs==rhs):
                    t=(j[0],i,j[1])
                    k012.add(t)
    k012_t=list(k012)
...
def findKeyspace():
    for i in range(len(keys)):
        ...
        ctr=0
        found = 0
        for j in keyspace:              # Just for verification's sake checking if the actual key is there or not
            ctr+=1
            if j==keys[i]:
                found = 1
    ...
    print()
# In[ ]:
findKeyspace() #Calling the function which does the actual work
